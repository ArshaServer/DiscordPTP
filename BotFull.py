...
api_key = 'AIzaSyA7xPeef3O5bYL3PaEnHsWJ3EXyM8qJ3wU'
...
async def on_message(message):  #alle ifs bei unterschliedlichen commands 
    ...
    if message.author.bot:      
        return
    if '!pet the peepo' in message.content:
        ...
        await message.channel.send('pet the peepo view: ' + response['items'][0]['statistics']['viewCount'])
    ...
    member = discord.utils.get(message.guild.members, name="Salkei")
    member = discord.utils.get(message.guild.members, name="Punikin")
    member2 = discord.utils.get(message.guild.members, name="Ƭhalthყ")
    member = discord.utils.get(message.guild.members, name="Salkei")
    if message.author == member:        
        await message.add_reaction('<:UGAY:642807039780716604>')
        await message.add_reaction('<:YouBudhist:654406169770590238>')
    if message.author == member2:        
        await message.add_reaction('<:UGAY:642807039780716604>')
        await message.add_reaction('<:thalthy:642806635575640104>')
    if commands[0] in message.content: #gayTester
        member = discord.utils.get(message.guild.members, name="Madafii")
        if message.author == member:  
...
client.run("Njc4MDE4MjcyNDA2NDA1MTIz.XkcruQ.BIpN5pi23RvUIGUQJH1xFdxS-cQ")
